...
// NetworkManager.swift - BŐVÍTVE

extension NetworkManager {
    func createPostWithAudio(content: String?, audioData: AudioData?, userId: Int, completion: @escaping (Result<Int, Error>) -> Void) {
        guard let url = URL(string: "\(baseURL)/posts") else {
            completion(.failure(NetworkError.invalidURL))
            return
        }
        ...
        // 👇 ÚJ: Audio adatok hozzáadása, ha vannak
        if let audioData = audioData {
            parameters["audio_url"] = audioData.url
            parameters["audio_title"] = audioData.title
            parameters["audio_artist"] = audioData.artist
            parameters["audio_duration"] = audioData.duration
        }
