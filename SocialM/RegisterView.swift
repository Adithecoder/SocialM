        ...
        NavigationView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 16) {
                    ...
                    TextField(NSLocalizedString("register.username", comment: "Username field"), text: $username)                        .font(.custom("OrelegaOne-Regular", size: 18))
                        .padding()
                    ...
                    TextField(NSLocalizedString("register.email", comment: "Email field"), text: $email)
                    ...
                    SecureField(NSLocalizedString("register.password", comment: "Password field"), text: $password)
                        .font(.custom("OrelegaOne-Regular", size: 18))
                        .padding()
                        .overlay(
                            RoundedRectangle(cornerRadius: 20)
                    ...
                    SecureField(NSLocalizedString("register.confirm_password", comment: "Confirm password field"), text: $confirmPassword)
                        .font(.custom("OrelegaOne-Regular", size: 18))
                        .padding()
                        .overlay(
                            RoundedRectangle(cornerRadius: 20)
...
//                    .frame(width: 200, height: 200)
                VStack {
                    Text(NSLocalizedString("register.terms", comment: "Registration terms"))
                        .font(.caption)
                        .foregroundColor(.gray)
                }
                .padding(.top,200)
...
//                }
            ...
            .alert(isPresented: $showAlert) {
                Alert(
                    title: Text(NSLocalizedString("alert.registration", comment: "Registration alert title")),
    ...
    private func register() {
        guard isFormValid else {
            alertMessage = NSLocalizedString("register.validation.fill_all", comment: "Fill all fields validation")
            showAlert = true
            return
        }

        guard password == confirmPassword else {
            alertMessage = NSLocalizedString("register.validation.passwords_match", comment: "Passwords don't match validation")
            showAlert = true
            return
        }
        ...
        NetworkManager.shared.register(username: username, email: email, password: password) { result in
            DispatchQueue.main.async {
                ...
                case .success(let registerResponse):
                    ...
                    if registerResponse.message == NSLocalizedString("register.server.success", comment: "Successful registration") {
                        // Sikeres regisztráció után automatikus bejelentkezés
                        self.debugInfo = NSLocalizedString("register.server.success", comment: "Successful registration")
